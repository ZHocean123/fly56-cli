...
class CollectDirectUrl extends command_1.Command {
    async run() {
        ...
        files.forEach(filePath => {
            ...
            // axios url
            // 直采
            const directUrls = fileContent.match(/(?<=minRequest\.\S*\({\s*url:\s*')\S*(?=')/g);
            if (directUrls) {
                urls.push(...directUrls === null || directUrls === void 0 ? void 0 : directUrls.map(url => '直采,' + url.split('?').shift()));
            }
            // 飞象
            const flyUrls = fileContent.match(/(?<=feixiangBaseUrl\s*\+\s')\S*(?=')/g);
            if (flyUrls) {
                urls.push(...flyUrls === null || flyUrls === void 0 ? void 0 : flyUrls.map(url => '飞象,' + url.split('?').shift()));
            }
        });
